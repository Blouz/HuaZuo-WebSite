...
class ModelExtensionModuleNitroHtaccess extends ModelExtensionModuleNitro {
    ...
    public function applyHtaccessCompressionRules() {
        ...
        if (getNitroPersistence('Compress.CSS') && (int)getNitroPersistence('Compress.CSSLevel') > 0 && !NITRO_ALTERNATIVE_CSS_COMPRESS) {
            ...
            $htrules .= 'RewriteRule ^(\/?(catalog).+)\.css$ ' . $assets_relative_path . '/style.php?l=' . getNitroPersistence('Compress.CSSLevel') . '&p=$1' . (!empty($browser_cache) ? '&c=' . $browser_cache : '') . ' [NC,L]'.PHP_EOL;
            //$htrules .= '</IfModule>'.PHP_EOL;
        }
        ...
        if (getNitroPersistence('Compress.JS') && (int)getNitroPersistence('Compress.JSLevel') > 0 && !NITRO_ALTERNATIVE_JS_COMPRESS) {
            ...
            //$htrules .= '<IfModule mod_rewrite.c>'.PHP_EOL;
            $htrules .= 'RewriteCond %{SCRIPT_FILENAME} !-d'.PHP_EOL;
            $htrules .= 'RewriteRule ^(\/?(catalog).+)\.js$ ' . $assets_relative_path . '/script.php?l=' . getNitroPersistence('Compress.JSLevel') . '&p=$1' . (!empty($browser_cache) ? '&c=' . $browser_cache : '') . ' [NC,L]'.PHP_EOL;
            //$htrules .= '</IfModule>'.PHP_EOL;
        }
