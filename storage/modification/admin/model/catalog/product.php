...
class ModelCatalogProduct extends Model {
      ...
      if (!empty($data['gkd_extra_desc_fields']) && defined('GKD_UNIV_IMPORT')) {
        ...
        // >> Parent-child Options
				...
				if ( isset($product_option['poip_settings']) ) {
					$this->liveopencart_poip->getModel()->setRealProductSettings($product_id, $product_option_id, $product_option['poip_settings']);
				}
				// >> Product Option Image PRO module
				...
				if (isset($product_option_value['images'])) {
					$this->liveopencart_poip->getModel()->saveProductOptionValueImages($product_id, $product_option_id, $product_option_value_id, $product_option_value['images']);
				}
		...
		if (isset($data['product_special'])) {
			...
			// >> Live Price
				...
				if ( isset($data['product_image']) && (__FUNCTION__ == 'addProduct' || __FUNCTION__ == 'editProduct') ) {
					liveopencart\poip::initLibrary($this->registry);
					$this->liveopencart_poip->getModel()->saveOptionsForImages($product_id, $data['product_image']);
				}
	...
	public function editProduct($product_id, $data) {
      ...
      if (!empty($data['gkd_extra_desc_fields']) && defined('GKD_UNIV_IMPORT')) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_option WHERE product_id = '" . (int)$product_id . "'");
				...
				// Product Option Image PRO module <<
				liveopencart\poip::initLibrary($this->registry);
				$this->liveopencart_poip->getModel()->deleteProductOptionValueImages($product_id);
				$this->liveopencart_poip->getModel()->deleteRealProductSettings($product_id);
				// >> Product Option Image PRO module
	...
	public function getProductAttributes($product_id) {
        ...
        public function pcop_checkTables() {
          $this->load->model('extension/module/parent_child_options');
					$this->model_extension_module_parent_child_options->checkTables();
        }
        
        public function pcop_getParents($product_option_id) {
          $this->load->model('extension/module/parent_child_options');
	...
	public function getProductOptions($product_id) {
				...
				$images 								= $this->liveopencart_poip->getModel()->getProductOptionImages($product_id);
				$product_poip_settings 	= $this->liveopencart_poip->getModel()->getRealProductSettings($product_id);
				// >> Product Option Image PRO module
	...
	public function getProductImages($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "' ORDER BY sort_order ASC");
				...
				$query->rows = $this->liveopencart_poip->getModel()->addOptionsDataToAdditionalImages( $product_id, $query->rows );
