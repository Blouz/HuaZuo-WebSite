...
class ControllerExtensionModuleiSenseLabsSeo extends Controller {
	...
	public function index() {
        ...
        $this->document->addScript('view/javascript/'.$this->moduleName.'/charts/d3.v3.min.js'); 
        $this->document->addScript('view/javascript/'.$this->moduleName.'/charts/c3.min.js'); 
        ...
        // For the autolinks
        $this->document->addScript('view/javascript/'.$this->moduleName.'/autolinks.js'); 

        // For the custom URLs
        $this->document->addScript('view/javascript/'.$this->moduleName.'/customurls.js'); 
        
        // For the image zoom
        $this->document->addScript('view/javascript/'.$this->moduleName.'/ekko_lightbox.js'); 
        
        // For the advanced editor
        $this->document->addScript('view/javascript/'.$this->moduleName.'/advanced_editor.js'); 

        // Styles		
		$this->document->addStyle('view/stylesheet/'.$this->moduleName.'/'.$this->moduleName.'.css');	
        $this->document->addStyle('view/javascript/'.$this->moduleName.'/charts/c3.min.css'); 
        $this->document->addStyle('view/stylesheet/'.$this->moduleName.'/ekko_lightbox.css'); 
        // Store Data
		if(!isset($this->request->get['store_id'])) {
           $this->request->get['store_id'] = 0;
        } 
		$this->data['store'] = $this->getCurrentStore($this->request->get['store_id']);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link($this->modulePath, $this->data['token_string'] . '=' . $this->data['token'] . '&store_id='.$this->data['store']['store_id'], 'SSL'));
		}

        $this->data['toggle_seo_analysis'] = $this->moduleModel->getSetting('search_engine_analytics_enable');
        $this->data['toggle_seo_analysis_url'] = $this->url->link($this->modulePath . '/toggle_seo_analysis', $this->data['token_string'] . '=' . $this->data['token'] . '&store_id='.$this->data['store']['store_id'], 'SSL');

        $this->data['toggle_404_detection'] = $this->moduleModel->getSetting('404_pages_gathering');
        $this->data['toggle_404_detection_url'] = $this->url->link($this->modulePath . '/toggle_missing_pages_detection', $this->data['token_string'] . '=' . $this->data['token'] . '&store_id='.$this->data['store']['store_id'], 'SSL');

        $this->data['canonical_products']           = $this->moduleModel->getSetting('canonical_products');
        $this->data['canonical_information_pages']  = $this->moduleModel->getSetting('canonical_information_pages');
        $this->data['canonical_home_page']          = $this->moduleModel->getSetting('canonical_home_page');
        $this->data['canonical_special_page']       = $this->moduleModel->getSetting('canonical_special_page');
        $this->data['canonical_manufacturers']      = $this->moduleModel->getSetting('canonical_manufacturers');
        $this->data['canonical_categories']         = $this->moduleModel->getSetting('canonical_categories');
		...
		$this->data['action']					= $this->url->link($this->modulePath, $this->data['token_string'] . '=' . $this->data['token'], 'SSL');
		$this->data['cancel']					= $this->extensionsLink;
        $this->data['config_language_id']       = $this->config->get('config_language_id');
        $this->data['moduleSettings']			= $this->model_setting_setting->getSetting($this->moduleName, $this->data['store']['store_id']);
    ...
    public function save_settings() {
        ...
        if (!empty($settings)) {
            ...
            if (isset($settings['htaccess'])) {
                $settings['htaccess'] = trim(html_entity_decode($settings['htaccess']));
                ...
                $settings['robots_txt'] = trim(html_entity_decode($settings['robots_txt']));
                ...
                foreach ($settings as $key => $value) {
                    if (!empty($key) && !is_null($value)) {
                        $this->moduleModel->saveSetting($key, $value);
                    }
                }
                $json['success'] = true;
                $json['message'] = $this->data['success_saved_settings'];
    ...
    public function fix_seo_issues() {
        ...
        switch($event_type) {
            case "migrate_seo_urls":
                $result = $this->moduleModel->migrateOldSeoUrlsToNewTable();
                if ($result) {
                    $json['message'] = $this->data['success_migration_text'];
                    $json['success'] = true;
                } else {
                    $this->error['warning'] = $this->data['error_unexpected'] . 'ERR_MSU';
                } 
                echo json_encode($json); exit;
                break;
            case "config_seo_url":
                $result = $this->moduleModel->enableSEOUrls();
                if ($result) {
            ...
            case "category_seo_urls":
                $result = $this->moduleModel->createSEOUrls('categories', $language_id);
                if ($result) {
            ...
            case "category_seo_urls_all":
                $result = $this->moduleModel->createSEOUrls('categories_all', $language_id);
                if ($result) {
                    $json['message']    = $result['counter'].' category links were created!';
                    $json['items']      = $result['items'];
                    $json['success']    = true;
            ...
            case "product_seo_urls":
                $result = $this->moduleModel->createSEOUrls('products', $language_id);
                if ($result) {
            ...
            case "product_seo_urls_all":
                $result = $this->moduleModel->createSEOUrls('products_all', $language_id);
                if ($result) {
                    $json['message']    = $result['counter'].' product links were created!';
                    $json['items']      = $result['items'];
                    $json['success']    = true;
            ...
            case "manufacturer_seo_urls":
                $result = $this->moduleModel->createSEOUrls('manufacturers', $language_id);
                if ($result) {
            ...
            case "manufacturer_seo_urls_all":
                $result = $this->moduleModel->createSEOUrls('manufacturers_all', $language_id);
                if ($result) {
                    $json['message']    = $result['counter'].' manufacturer links were created!';
                    $json['items']      = $result['items'];
                    $json['success']    = true;
            ...
            case "information_seo_urls":
                $result = $this->moduleModel->createSEOUrls('informations', $language_id);
                if ($result) {
            ...
            case "information_seo_urls_all":
                $result = $this->moduleModel->createSEOUrls('informations_all', $language_id);
                if ($result) {
                    $json['message']    = $result['counter'].' information links were created!';
                    $json['items']      = $result['items'];
                    $json['success']    = true;
    ...
    public function generate_image_names() {
        $result = $this->moduleModel->generateImageNames();
    ...
    public function tab_titles() {
        ...
        $this->data['seo_data']                 = $this->moduleModel->checkSEOTitlesCount();
    ...
    public function tab_descriptions() {
        ...
        $this->data['seo_data']                 = $this->moduleModel->checkSEODescriptionsCount();
    ...
    public function tab_keywords() {
        ...
        $this->data['seo_data']                 = $this->moduleModel->checkSEOKeywordsCount();
    ...
    public function tab_image_titles() {
        ...
        if (!empty($last_activity_data)) {
            $this->data['seo_image_last_activity'] = json_decode($last_activity_data, true);
        } else {
    ...
    public function tab_seo_links() {
        ...
        $this->data['seo_link_data']            = $this->moduleModel->checkSEOUrlsCount();
        
        $this->data['product_url_string']       = $this->moduleModel->getSetting('product_url_string');
        $this->data['category_url_string']      = $this->moduleModel->getSetting('category_url_string');
        $this->data['manufacturer_url_string']  = $this->moduleModel->getSetting('manufacturer_url_string');
        $this->data['information_url_string']   = $this->moduleModel->getSetting('information_url_string');
    ...
    public function tab_home() {
		...
		$this->data['store']                    = $this->getCurrentStore($this->request->get['store_id']);
        $this->data['seo_data']                 = $this->moduleModel->getSeoScore();
        $this->data['seo_score']                = $this->moduleModel->getSetting('seo_score');
        $this->data['seo_score_last_checked']   = $this->moduleModel->getSetting('seo_score_last_checked');
    ...
    public function calculate_seo_score() {
        $json   = array();
        $data   = $this->moduleModel->getSeoScore();       
    ...
    public function tab_autolinks() {
        ...
        $this->data['total']        = $this->moduleModel->getTotalAutoLinks();
        $this->data['limit']        = 10;
        ...
        $pagination->url			= $this->url->link($this->modulePath.'/tab_autolinks', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}', 'SSL');
        
        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModel->getAutoLinks($page, $this->data['limit']);
    ...
    public function add_autolink() {
        ...
        if ($this->moduleModel->addAutoLink($data)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
    ...
    public function edit_autolink() {
        ...
        if ($this->moduleModel->editAutoLink($data, $id)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function tab_customurls() {
        ...
        $this->data['total']        = $this->moduleModel->getTotalCustomUrls();
        $this->data['limit']        = 10;
        ...
        $pagination->page			= $page;
        $pagination->limit			= $this->data['limit']; 
        $pagination->url			= $this->url->link($this->modulePath.'/tab_customurls', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}', 'SSL');

        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModel->getCustomUrls($page, $this->data['limit']);
    ...
    public function add_customurl() {
        ...
        if ($this->moduleModel->addCustomUrl($data)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function get_customurl() {
        $id         = $this->request->get['id'];
        $json       = array();
        $data       = $this->moduleModel->getCustomUrl($id);
        if ($data != false) {
            $json['success'] = true;
            $json['message'] = '';
            $json['data']    = $data;
    ...
    public function edit_customurl() {
        $json       = array();
        $id         = $this->request->get['id'];
        ...
        if ($this->moduleModel->editCustomUrl($data, $id)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function remove_customurl() {
        ...
        if ($this->moduleModel->deleteCustomUrl($id)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function remove_customurls() {
        ...
        foreach ($ids as $id) {
            if (!$this->moduleModel->deleteCustomUrl($id['value'])) { 
                $flag = false;
            }
        }
    ...
    public function get_crawler_data() {
        ...
        if (empty($response)) {
            ...
            $start_time = microtime(true);
            if (!$site_contents = @file_get_contents($url)) {
                $error = error_get_last();
                $this->data['display_data'] = false; 
                echo ($this->load->view($this->modulePath.'/tab_crawler_results.tpl', $this->data));
                exit;
            } 
            $end_time = microtime(true);
    ...
    public function tab_seo_analysis() {
        ...
        $this->data['total']        = $this->moduleModel->getTotalCrawledUrls($crawler, $url, $date_start, $date_end);
        $this->data['limit']        = 15;
        ...
        $pagination->page			= $page;
        $pagination->limit			= $this->data['limit']; 
        $pagination->url			= $this->url->link($this->modulePath.'/tab_seo_analysis', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}'.$filter_url, 'SSL');

        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModel->getCrawledUrls($page, $crawler, $url, $date_start, $date_end, $this->data['limit']);
    ...
    public function clear_seo_analysis() {
        ...
        if ($this->moduleModel->clearSeoAnalysisResults()) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function toggle_seo_analysis() {
        ...
        $this->response->redirect($this->url->link($this->modulePath, $this->data['token_string'] . '=' . $this->data['token'], 'SSL'));
    }
    
    public function save_editor_field() {
        $json = array();
        
        if ($this->moduleModelEditor->saveField($this->request->post)) {
            $json['success'] = true;
        } else {
    ...
    public function editor_products() {
        ...
        $this->data['total']        = $this->moduleModelEditor->getTotalProducts($data);
        $this->data['action_url']   = $this->url->link($this->modulePath.'/editor_products', $this->data['token_string'] . '=' . $this->data['token'], 'SSL');
        
        $pagination					= new Pagination();
        $pagination->total			= $this->data['total'];
        $pagination->page			= $page;
        $pagination->limit			= $filter_limit; 
        $pagination->url			= $this->url->link($this->modulePath.'/editor_products', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}' . $url, 'SSL');
        
        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModelEditor->getProducts($page, $data);
    ...
    public function editor_categories() {
        ...
        $this->data['total']        = $this->moduleModelEditor->getTotalCategories($data);
        $this->data['language_id']  = $language_id;
        $this->data['action_url']   = $this->url->link($this->modulePath.'/editor_categories', $this->data['token_string'] . '=' . $this->data['token'], 'SSL');
        
        $pagination					= new Pagination();
        $pagination->total			= $this->data['total'];
        $pagination->page			= $page;
        $pagination->limit			= $filter_limit; 
        $pagination->url			= $this->url->link($this->modulePath.'/editor_categories', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}' . $url, 'SSL');
        
        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModelEditor->getCategories($page, $data);
        $this->data['limit']        = $filter_limit;
    ...
    public function editor_informations() {
        ...
        $this->data['total']        = $this->moduleModelEditor->getTotalInformations($data);
        $this->data['language_id']  = $language_id;
        $this->data['action_url']   = $this->url->link($this->modulePath.'/editor_informations', $this->data['token_string'] . '=' . $this->data['token'], 'SSL');
        
        $pagination					= new Pagination();
        $pagination->total			= $this->data['total'];
        $pagination->page			= $page;
        $pagination->limit			= $filter_limit; 
        $pagination->url			= $this->url->link($this->modulePath.'/editor_informations', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}' . $url, 'SSL');
        
        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModelEditor->getInformations($page, $data);
        $this->data['limit']        = $filter_limit;
    ...
    public function editor_manufacturers() {
        ...
        $this->data['total']        = $this->moduleModelEditor->getTotalManufacturers($data);
        $this->data['language_id']  = $language_id;
        $this->data['action_url']   = $this->url->link($this->modulePath.'/editor_manufacturers', $this->data['token_string'] . '=' . $this->data['token'], 'SSL');
        
        $pagination					= new Pagination();
        $pagination->total			= $this->data['total'];
        $pagination->page			= $page;
        $pagination->limit			= $filter_limit; 
        $pagination->url			= $this->url->link($this->modulePath.'/editor_manufacturers', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}' . $url, 'SSL');
        
        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModelEditor->getManufacturers($page, $data);
        $this->data['limit']        = $filter_limit;
    ...
    public function tab_detected_missing_pages() {
        ...
        $this->data['total']        = $this->moduleModel->getTotalDetectedPages();
        $this->data['limit']        = 10;
        ...
        $pagination->page			= $page;
        $pagination->limit			= $this->data['limit']; 
        $pagination->url			= $this->url->link($this->modulePath.'/tab_detected_missing_pages', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}', 'SSL');

        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModel->getDetectedPages($page, $this->data['limit']);
    ...
    public function remove_detected_page() {
        ...
        if ($this->moduleModel->deleteDetectedPage($id)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function remove_detected_pages() {
        ...
        foreach ($ids as $id) {
            if (!$this->moduleModel->deleteDetectedPage($id['value'])) { 
                $flag = false;
            }
        }
    ...
    public function add_missing_page_redirect() {
        ...
        if ($this->moduleModel->addMissingPageRedirect($data)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function tab_redirects() {
        ...
        $this->data['total']        = $this->moduleModel->getTotalRedirects();
        $this->data['limit']        = 10;
        ...
        $pagination->page			= $page;
        $pagination->limit			= $this->data['limit']; 
        $pagination->url			= $this->url->link($this->modulePath.'/tab_redirects', $this->data['token_string'] . '=' . $this->data['token'] . '&page={page}', 'SSL');

        $this->data['page']         = $page;
		$this->data['pagination']   = $pagination->render();
        $this->data['sources']      = $this->moduleModel->getRedirects($page, $this->data['limit']);
    ...
    public function remove_redirect() {
        ...
        if ($this->moduleModel->deleteRedirect($id)) {
            $json['success'] = true;
            $json['message'] = '';
        } else {
            $json['error']   = $this->data['error_unexpected'];
    ...
    public function remove_redirects() {
        ...
        foreach ($ids as $id) {
            if (!$this->moduleModel->deleteRedirect($id['value'])) { 
                $flag = false;
            }
        }
    ...
    public function install() {
        ...
        $this->model_setting_event->addEvent($this->eventGroup, "catalog/controller/error/not_found/before", $this->modulePath . "/notFoundPageHandler");
