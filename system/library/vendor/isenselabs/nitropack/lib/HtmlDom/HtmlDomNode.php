...
class HtmlDomNode {
    ...
    public function parseDom(&$iterator) {
        ...
        foreach ($iterator as $char) {
            ...
            if ($this->tagName == 'script' || $this->tagName == 'style' || $this->tagName == 'noscript') {
                if ($char == '<') {
                    ...
                    if ($this->tagName == 'noscript' && strtolower($iterator->peek(9)) == '/noscript') {
                        //TODO: maybe an $iterator->consume statement will increase things up a bit
                        foreach ($iterator as $subchar) {//read untill the closing > to handle cases like </noscript wtf>
                            if ($subchar == '>') return;
                        }
                    }
