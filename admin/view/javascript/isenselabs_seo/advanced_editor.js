...
$(document).ready(function(e) {
    ...
    $.ajax({
        url: "index.php?route=" + modulePath + "/editor_products&" + token_string + "=" + token,
        type: 'get',
        dataType: 'html',
    ...
    $.ajax({
        url: "index.php?route=" + modulePath + "/editor_categories&" + token_string + "=" + token,
        type: 'get',
        dataType: 'html',
        success: function(data) { 
    ...
    $.ajax({
        url: "index.php?route=" + modulePath + "/editor_informations&" + token_string + "=" + token,
        type: 'get',
        dataType: 'html',
        success: function(data) { 
    ...
    //Advanced Editor Informations
    $.ajax({
        url: "index.php?route=" + modulePath + "/editor_manufacturers&" + token_string + "=" + token,
        type: 'get',
        dataType: 'html',
        success: function(data) { 
...
function editField(id, type, selector) {
    if ($(selector).find('input').length > 0) { return false; }

    var textValue = $(selector).find('span.editable').html().trim();
    ...
    var editableArea = '<div class="input-group">';
    editableArea += '<input type="text" value="' + textValue + '" class="edit-field-input form-control">';
    editableArea += '<div class="input-group-btn"><a onClick="cancelField(\'' + placement + '\',\''+ textValue + '\');" class="btn btn-default"><i class="fa fa-times" aria-hidden="true"></i></a><a onClick="saveField(' + id + ',\'' + type + '\',\'' + placement + '\',\''+ entityType + '\',' + language_id + ',\'' + textValue + '\',this);" class="btn btn-primary"><i class="fa fa-check" aria-hidden="true"></i></a></div>';
    editableArea += '</div>';
    
    $(selector).html(editableArea);
}
...
function saveField(id, type, placement, entityType, language_id, old_text, selector) {
    var newData = $(selector).parents('.input-group').find('input').val().trim();
    
    if (newData.length > 0 && newData != old_text) {
        $.ajax({
