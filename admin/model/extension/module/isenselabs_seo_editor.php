...
class ModelExtensionModuleiSenseLabsSeoEditor extends Model {  
    ...
    public function getProducts($page = 1, $data = array()) {
        ...
        $query .= " LEFT JOIN `" . DB_PREFIX . "seo_url` sua ON (CONCAT('product_id=',p.product_id) = sua.query AND sua.language_id = '" . $data['language_id'] . "')";
        ...
        $query .= " ORDER BY `p`.`product_id` DESC LIMIT ".$start.", ".$data['filter_limit'];
    ...
    public function getCategories($page = 1, $data = array()) {
        ...
        $query .= " LEFT JOIN `" . DB_PREFIX . "seo_url` sua ON (CONCAT('category_id=',c.category_id) = sua.query AND sua.language_id = '" . $data['language_id'] . "')";
        ...
        $query .= " ORDER BY `c`.`category_id` DESC LIMIT ".$start.", ".$data['filter_limit'];
    ...
    public function getInformations($page = 1, $data = array()) {
        ...
        $query .= " LEFT JOIN `" . DB_PREFIX . "seo_url` sua ON (CONCAT('information_id=',i.information_id) = sua.query AND sua.language_id = '" . $data['language_id'] . "')";
        
        $query .= " WHERE 1=1 ";
        
        if (!empty($data['filter_name'])) {
            $query .= " AND id.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
        }
        ...
        $query .= " ORDER BY `i`.`information_id` DESC LIMIT ".$start.", ".$data['filter_limit'];
    ...
    public function getManufacturers($page = 1, $data = array()) {
        ...
        $query .= " LEFT JOIN `" . DB_PREFIX . "seo_url` sua ON (CONCAT('manufacturer_id=',m.manufacturer_id) = sua.query AND sua.language_id = '" . $data['language_id'] . "')";
        ...
        $query .= " ORDER BY `m`.`manufacturer_id` DESC LIMIT ".$start.", ".$data['filter_limit'];
    ...
    public function saveField($data = array()) {
        if (!empty($data) && !empty($data['id']) && !empty($data['field']) && !empty($data['language_id']) && !empty($data['entity_type']) && !empty($data['value'])) {
            ...
            if (!empty($field) && !empty($used_table)) {
                ...
                if ($data['field'] == 'url_alias') {
                    $check_if_exists = $this->db->query("SELECT * FROM `" . DB_PREFIX . $used_table . "` WHERE `" . $field . "` = '" . trim($this->db->escape($data['value'])) . "'");
