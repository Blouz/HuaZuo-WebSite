...
class Tar extends Archive
{
     ...
     */
    public function open($file, $append = false)
    {
        $this->file = $file;
        ...
        if ($this->comptype === Archive::COMPRESS_GZIP) {
            ...
            if ($append) {
                $this->fh = @fopen($this->file, 'cb+');
                if (is_numeric($append)) {
                    fseek($this->fh,$append, SEEK_SET);
                } else {
                    fseek($this->fh, -1024, SEEK_END);
     ...
     * @return FileInfo[]
     */
    public function extract($outdir, $strip = '', $exclude = '', $include = '', $num_files)
    {
        if ($this->closed || !$this->file) {
            throw new ArchiveIOException('Can not read from a closed archive');
        }
        ...
        while ($dat = $this->readbytes(512)) {
            ...
            $extracted[] = $fileinfo;
            if ($num_extracted ==$num_files) {
                break;   
            }
        }
        $extracted['current_position'] = ftell($this->fh);
        //$this->close();
        return $extracted;
    }
