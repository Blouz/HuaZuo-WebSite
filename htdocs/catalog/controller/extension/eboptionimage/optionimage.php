...
class ControllerExtensionEboptionimageOptionimage extends Controller {
	public function index() {
		...
		if($row['image']){
			if(!$notimage){
				$notimage = true;
				$data['thumb'] = $this->model_tool_image->resize($row['image'],$this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
			}
			$data['images'][] = array(
				...
				'thumb' => $this->model_tool_image->resize($row['image'],$this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height')),
			);
		}
		endforeach;
