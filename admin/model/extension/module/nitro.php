<?php
class ModelExtensionModuleNitro extends Model {
    ...
    private $event_group_sidebar = "nitropack_sidebar";
    ...
    public function setupEventHandlers() {
        ...
        $this->model_setting_event->deleteEventByCode($this->event_group_sidebar);

        // Admin events
        $this->model_setting_event->addEvent($this->event_group_sidebar, "admin/view/common/column_left/before", $this->module_path . "/injectAdminMenuItem");
        ...
        $this->model_setting_event->addEvent($this->event_group, "catalog/model/tool/image/resize/before", $this->module_path . "/automaticImageDimensionOverride");
        ...
        $this->model_setting_event->addEvent($this->event_group, "catalog/view/common/maintenance/before", $this->module_path . "/disableMaintenanceIfNecessary");
    ...
    public function tryChmod() {
        ...
        $nitro_folders = array(
            NITRO_ASSETS_FOLDER . 'css',
            NITRO_ASSETS_FOLDER . 'js',
            NITRO_FOLDER . 'data',
            NITRO_FOLDER . 'temp',
    ...
    public function clearJSCache($ajax = false) {
        return $this->trunc_folder(NITRO_ASSETS_FOLDER . 'js', 'index.html', $ajax);
    }
    ...
    public function clearCSSCache($ajax = false) {
        return $this->trunc_folder(NITRO_ASSETS_FOLDER . 'css', 'index.html', $ajax);
    }
    ...
    public function applyNitroCacheHTRules() {
        $this->load->model('extension/module/nitro_htaccess');
        $this->model_extension_module_nitro_htaccess->applyHtaccessRules();
    }

    public function applyNitroCacheHTCompressionRules() {
        $this->load->model('extension/module/nitro_htaccess');
        $this->model_extension_module_nitro_htaccess->applyHtaccessCompressionRules();
    }

    public function applyNitroCacheHTCookieRules() {
        $this->load->model('extension/module/nitro_htaccess');
        $this->model_extension_module_nitro_htaccess->applyHtaccessCookieRules();
    }

    public function applyNitroCacheHTCdnRules() {
        $this->load->model('extension/module/nitro_htaccess');
        $this->model_extension_module_nitro_htaccess->applyHtaccessCdnRules();
    }
    ...
    public function getServerInfo($permission) {
        ...
        if (function_exists('apache_get_modules')) {
            ...
            $shell_exec_enabled = NITRO_FORCE_ENABLE_EXEC || (
        ...
        if (!$permission) $result['path_assets_css'] = $text_no_permission;
            ...
            if (file_exists(NITRO_ASSETS_FOLDER . 'css') && is_writable(NITRO_ASSETS_FOLDER . 'css')) $result['path_assets_css'] = '<span class="info-success"><strong>Writable</strong></span>';
            else if (!file_exists(NITRO_ASSETS_FOLDER . 'css')) $result['path_assets_css'] = '<span><strong>Does not exist.</strong></span>';
        ...
        if (!$permission) $result['path_assets_js'] = $text_no_permission;
            ...
            if (file_exists(NITRO_ASSETS_FOLDER . 'js') && is_writable(NITRO_ASSETS_FOLDER . 'js')) $result['path_assets_js'] = '<span class="info-success"><strong>Writable</strong></span>';
            else if (!file_exists(NITRO_ASSETS_FOLDER . 'js')) $result['path_assets_js'] = '<span><strong>Does not exist.</strong></span>';
    ...
    public function cron_command($config, $full_command = true) {
        ...
        $cron_command = array(
            ...
            'command' => 'export nitro_server_name="' . $server_name . '"; ' . $cmd . ' '. NITRO_CORE_FOLDER . 'cron.php >/dev/null 2>&1'
        );
    ...
    public function getProductByStoreId($store_id) {
        $sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store pts ON (p.product_id = pts.product_id) WHERE pts.store_id = '" . (int)$store_id . "' AND p.status='1' LIMIT 1";
    ...
    public function getCategoryByStoreId($store_id) {
        $sql = "SELECT * FROM " . DB_PREFIX . "product p 
            ...
            LEFT JOIN " . DB_PREFIX . "category_to_store cts ON (c.category_id = cts.category_id)
            WHERE pts.store_id = '" . (int)$store_id . "' AND p.status='1' AND cts.store_id = '" . (int)$store_id . "' AND c.status='1' LIMIT 1";
        return $this->db->query($sql)->row;
    }
