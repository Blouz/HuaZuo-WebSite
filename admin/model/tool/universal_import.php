...
class ModelToolUniversalImport extends Model {
  ...
  public function process($config, $limit = null) {
    ...
    if (!in_array($type, array('product', 'product_update', 'order', 'order_status_update', 'category', 'information', 'manufacturer', 'customer'))) {
      die('Invalid type');
    }
  ...
  protected function optionHandler($field, &$config, &$line) {
    ...
    */
    ...
    foreach ($values_array as $current_key => &$value) {
      ...
      $option_type = $option_name = $option_price = $option_price_prefix = $option_required = $option_weight = $option_qty = $option_subtract = '';
      ...
      if ($this->filetype != 'xml' && !empty($config['option_fields']) && !$value) {
        ...
        if (isset($config['option_fields']['price']) && !empty($line[$config['option_fields']['price']])) {
          $option_price =  $line[$config['option_fields']['price']];
        }

        if (isset($config['option_fields']['price_prefix']) && !empty($line[$config['option_fields']['price_prefix']])) {
          $option_price_prefix =  $line[$config['option_fields']['price_prefix']];
        }
        
        if (isset($config['option_fields']['required']) && !empty($line[$config['option_fields']['required']])) {
          $option_required =  $line[$config['option_fields']['required']];
        }
        
        if (isset($config['option_fields']['quantity']) && !empty($line[$config['option_fields']['quantity']])) {
          $option_qty =  $line[$config['option_fields']['quantity']];
        }
        
        if (isset($config['option_fields']['subtract']) && !empty($line[$config['option_fields']['subtract']])) {
          $option_subtract = $line[$config['option_fields']['subtract']];
        }
        
        if (isset($config['option_fields']['weight']) && !empty($line[$config['option_fields']['weight']])) {
          $option_weight = $line[$config['option_fields']['weight']];
        }
        
        // set defaults if empty
        if (empty($option_type)) {
          $option_type = isset($config['option_fields_default']['type']) ? $config['option_fields_default']['type'] : 'select';
        }
        ...
        if (empty($option_price)) {
          $option_price = isset($config['option_fields_default']['price']) ? $config['option_fields_default']['price'] : '';
        }
        
        if (empty($option_price_prefix)) {
          $option_price_prefix = isset($config['option_fields_default']['price_prefix']) ? $config['option_fields_default']['price_prefix'] : '';
        }
        
        if (empty($option_required)) {
          $option_required = isset($config['option_fields_default']['required']) ? $config['option_fields_default']['required'] : '';
        }
        
        if (empty($option_qty)) {
          $option_qty = isset($config['option_fields_default']['quantity']) ? $config['option_fields_default']['quantity'] : '';
        }
        
        if (empty($option_subtract)) {
          $option_subtract = isset($config['option_fields_default']['subtract']) ? $config['option_fields_default']['subtract'] : '';
        }
        
        if (empty($option_weight)) {
          $option_weight = isset($config['option_fields_default']['weight']) ? $config['option_fields_default']['weight'] : '';
        }
      }
      ...
      // xml advanced option fields
        ...
        if (!empty($config['option_fields']['price'])) {
          $option_price = $this->getArrayPath($value, $config['option_fields']['price']);
        }
        
        if (!empty($config['option_fields']['price_prefix'])) {
          $option_price_prefix = $this->getArrayPath($value, $config['option_fields']['price_prefix']);
        }
        
        if (isset($config['option_fields']['required']) && !empty($value[$config['option_fields']['required']])) {
          $option_required = $this->getArrayPath($value, $config['option_fields']['required']);
        }
        
        if (isset($config['option_fields']['quantity']) && !empty($value[$config['option_fields']['quantity']])) {
          $option_qty = $this->getArrayPath($value, $config['option_fields']['quantity']);
        }
        
        if (isset($config['option_fields']['subtract']) && !empty($value[$config['option_fields']['subtract']])) {
          $option_subtract = $this->getArrayPath($value, $config['option_fields']['subtract']);
        }
        
        if (isset($config['option_fields']['weight']) && !empty($value[$config['option_fields']['weight']])) {
          $option_weight = $this->getArrayPath($value, $config['option_fields']['weight']);
        }
      ...
      // set default values
      foreach (array('',) as $optType) {
        if (in_array($optType, array(''))) {
          
        } else if (empty(${'option_'.$optType})) {
          ${'option_'.$optType} = isset($config['option_fields_default'][$optType]) ? $config['option_fields_default'][$optType] : '';
        }
      }
  ...
  */
  ...
  protected function attributeHandler($field, &$config) {
    ...
    foreach ($values_array as $current_key => &$value) {
      // xml advanced values
      if ($this->filetype == 'xml' && !empty($config['attribute_fields']['value'][$this->config->get('config_language_id')])) {
  ...
  protected function storeHandler($field, &$config) {
    ...
    foreach ($values_array as $store) {
      if (is_numeric($store)) {
        ...
        if (array_search($store, $this->storeIdToName) !== false) {
