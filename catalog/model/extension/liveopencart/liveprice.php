...
class ModelExtensionLiveopencartLivePrice extends Model {

	private $extension_code = 'lppro3';
  ...
  public function getThemeName() {
	...
	public function getPathToMainJS() {
		return $this->getScriptPathWithVersion('view/theme/extension_liveopencart/live_price/liveopencart.live_price.js');
	}
	
	public function getPathToCustomJS() {
		return $this->getScriptPathWithVersion( $this->getDirOfCurrentTemplate().'code.js');
		//return $this->getScriptPathWithVersion('view/theme/extension_liveopencart/live_price/theme/'.$this->getThemeName().'/code.js');
	}
	/*
	public function getIncludePathForProductPage() {
		return DIR_APPLICATION.'view/extension/live_price/tpl/product_page.twig';
	}
	*/
	public function getBasicDirOfTemplates() {
		return DIR_TEMPLATE.'extension_liveopencart/live_price/theme/';
	}
	...
	public function getPriceTemplates() {
		...
		$files = array();
		$theme_dir = $this->getDirOfCurrentTemplate();
		$files = $this->getPriceTemplatesFromDirectory($theme_dir);

		/*
		$theme_dir_basic = $this->getBasicDirOfTemplates();
		
		$theme_dir = $this->getThemeName().'/';
		if ( file_exists($theme_dir_basic.$theme_dir) && is_dir($theme_dir_basic.$theme_dir) ) {
			$files = $this->getPriceTemplatesFromDirectory($theme_dir_basic, $theme_dir);
		}
		
		if ( !$files && $this->getThemeName() != 'default' ) {
			$theme_dir = 'default/';
			$files = $this->getPriceTemplatesFromDirectory($theme_dir_basic, $theme_dir);
		}
		*/
	...
	private function getPriceTemplatesFromDirectory($dir_theme) {
		...
		foreach ( glob($dir_theme.'*.twig') as $file_name ) {
			...
			//$files[basename($file_name, '.twig')] = dirname($file_name).'/'.basename($file_name, '.twig');
			//$files[basename($file_name, '.twig')] = $dir_theme.basename($file_name, '.twig');
		}
		return $files;
	}
	private function getTemplateFileRoute($file_name) {
		$route = dirname($file_name).'/'.basename($file_name, '.twig');
      ...
      if ( !empty($lp_settings['starting_from']) ) {
        $starting_prices = $this->getProductDefaultPrice($product_info['product_id'], true);
			...
			if ( !empty($lp_settings['show_from']) ) {
				...
				if ( ($lp_settings['show_from'] == 1 && !empty($starting_prices['minimal']))
				|| $lp_settings['show_from'] == 2
				|| ($lp_settings['show_from'] == 3 && $this->productHasOptions($product_info['product_id']))
				|| ($lp_settings['show_from'] == 4 && $this->productHasOptionsWithPrices($product_info['product_id'])) ) {
  ...
  function getProductDefaultPrice($product_id, $inclide_starting_from = false) {  
    ...
    if ( $this->installed() && $this->versionPRO() ) {
      ...
      $lp_data = $this->getProductPrice( $product_id, 1, array(), 0, false, true, true );
      ...
      if ( !empty($lp_settings['starting_from']) ) { 
        if ( $this->installedRelatedOptions() ) {
          ...
          if ( $this->installedRelatedOptions() )  {
            ...
            $ro_options = $this->model_extension_liveopencart_related_options->getProductROVariantOptions($product_id);
            
            $ro_ids = false;
            if ( $io_defaults ) {
              $ro_io_combs = $this->model_extension_liveopencart_related_options->getROCombsByPOIds($product_id, $io_defaults);
      ...
      $lp_data = $this->getProductPrice( $product_id, 1, $values_for_calc, 0, false, true, true );
  ...
  public function getProductDiscountQuery($product_id, $customer_group_id, $discount_quantity, $ro_combs=false) {
    ...
    if ( $this->installedRelatedOptions() ) { // installed, model loaded (for both)
  ...
  private function arrayKeysToInt($arr) {
	...
	public function getProductPriceByParamsArray($params) {
		...
		return $this->getProductPrice($product_id, $quantity, $options, $recurring_id, $multiplied_price, $use_cart_cache, $use_price_cache, $without_discounts, $ignore_cart, $qpo_discount_quantity_addition);
  ...
  public function getProductPrice($p_product_id, $current_quantity=1, $options=array(), $recurring_id=0, $multiplied_price=false, $use_cart_cache=false, $use_price_cache=false, $without_discounts=false, $ignore_cart=false, $qpo_discount_quantity_addition=0) {
    ...
    if ( $use_price_cache && isset($this->cache_price[$cache_price_key]) ) {
      ...
      if ( $settings_discount_quantity == 2 ) {
        ...
        if ( $this->installedRelatedOptions() ) {
          $ro_price_data = $this->model_extension_liveopencart_related_options->getROCombsByPOIds($product_id, $options, true);
        }
      }
            ...
            if ( $settings_discount_quantity==1 ) { // by options
              ...
              if ( isset($ro_price_data) && $ro_price_data ) {
								...
								if ($use_cart_cache && isset($cart_product['ro_price_data_cart'])) {
									...
									$ro_price_data_cart = $this->model_extension_liveopencart_related_options->getROCombsByPOIds($product_id, $cart_options, true);
      ...
      if ($product_query->num_rows) {
        ...
        // << for RO
				...
				if ( $this->installedRelatedOptions() ) {
					$ro_settings = $this->config->get('related_options');
					$ro_combs = $this->model_extension_liveopencart_related_options->getROCombsByPOIds($product_id, $options, true);
					if ($ro_combs) {
						$ro_custom_fields = $this->model_extension_liveopencart_related_options->getCustomFields($product_query->row, $ro_combs);
						if ( $ro_custom_fields ) {
        ...
        // << for RO 
				if ($ro_combs) {
					$ro_price_data = $this->model_extension_liveopencart_related_options->calcProductPriceWithRO($product_query->row['price'], $ro_combs,0,0,0,$quantity);
						...
						if ( !empty($lp_settings['percent_discount_to_total']) && !empty($discount['price_prefix']) && $discount['price_prefix'] != '=' ) {
							...
							if ( !empty($ro_discounts) ) {
								...
								if ($ro_combs) {
									...
									$ro_discount_data = $this->model_extension_liveopencart_related_options->calcProductPriceWithRO($discount_price, $ro_combs,0,0,0,$discount['quantity']);
	...
	public function getProductPageAdditionalData() {
		...
		$data['liveprice_installed'] = $this->model_extension_liveopencart_liveprice->installed();
		...
		if ( $data['liveprice_installed'] ) {
			...
			$data['lp_theme_name'] = $this->model_extension_liveopencart_liveprice->getThemeName();
