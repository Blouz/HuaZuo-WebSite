	...
	public function index() {
        ...
        // << Parent-child Options
				...
				if ( $this->parent_child_options_common->installed() ) {
					$this->journal2->minifier->addScript($this->parent_child_options_common->getProductPageScriptPath());
				}
		...
		if ($product_info) {
			...
			$results = $this->model_catalog_product->getProductImages($this->request->get['pid']);
				...
				$poip_scripts = array_merge( $this->liveopencart_poip->getModel()->getHeaderScripts(), $this->liveopencart_poip->getModel()->getProductPageScripts() );
				foreach ($poip_scripts as $poip_script) {
					$this->journal2->minifier->addScript($poip_script);
				}
				$poip_data = $this->liveopencart_poip->getModel()->getDataForProductPage($this->request->get['pid'], $results);
				if ( $poip_data ) {
