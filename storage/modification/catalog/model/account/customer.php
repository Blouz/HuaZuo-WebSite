...
class ModelAccountCustomer extends Model {
	...
	public function EditPicture($picture) {
		$query = $this->db->query("SHOW COLUMNS FROM `". DB_PREFIX ."customer` WHERE `Field` = 'picture'");
		if(!$query->num_rows) {
			$this->db->query("ALTER TABLE `". DB_PREFIX ."customer` ADD `picture` VARCHAR( 255 ) NOT NULL AFTER `email`");
		}

		$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET picture = '" . $this->db->escape($picture) . "' WHERE customer_id = '" . (int)$this->customer->getId() . "'");
	}
